...
const PostFormScreen = (props) => {
  ...
  useEffect(() => {
    ...
    async function fetchPostType() {
      try {
        ...
        let temp = { ...formInputs, isHidden: res.data.data.data.isHidden };
    ...
    if (mounted && !props.navigation.state.params.noReload) {
      fetchPostType();
      setMounted(false);
    }
  }, [mounted]);
  ...
  const onSubmitPost = async () => {
    ...
    tempInputs = {
      ...
      isHiddenFromFeed: props.navigation.state.params.isHiddenFromFeed
    };
    await onCreatePost(
      tempInputs,
      postTypeId,
